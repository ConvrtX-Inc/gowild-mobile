...
class TreasureWildUpCommingVC: UIViewController {
    ...
    deinit {
        Constants.printLogs("*** TreasureWildUpCommingVC Deinit ***")
    }
    ...
    private func navigateToEWavierVC(with title: String,id: String,eventPicture: String){
        guard let eWavierVC = R.storyboard.authSB.eWaiverVC() else {return}
        eWavierVC.mode = .treasureHunt
        eWavierVC.companyName = title
        eWavierVC.eventID = id
        eWavierVC.eventPicture = eventPicture
...
extension TreasureWildUpCommingVC: TreasureHuntViewModelDelegates{
...
extension TreasureWildUpCommingVC: CollectionViewMethods{
...
extension TreasureWildUpCommingVC{
        ...
        if let winnerID = treasureHunt.winnerId,
           !winnerID.isEmpty{
            ...
            if let currentUserHunt = treasureHunt.currentUserHunt{
                ...
                cell.registerBtn.setTitle(GoWildStrings.register().capitalized, for: .normal)
                cell.registerBtn.setTitleColor(AppColor.appWhiteColor(), for: .normal)
                cell.registerBtn.setBackgroundImage(R.image.loginBtnBgImage(), for: .normal)
                cell.registerBtn.addTapGestureRecognizer { [weak self] in
                    cell.registerBtn.showAnimation {
                        self?.navigateToEWavierVC(with: treasureHunt.title ?? "", id: treasureHunt.id ?? "", eventPicture: "\(UserManager.shared.getBaseURL())\(treasureHunt.picture ?? "")")
                    }
                }
            }
