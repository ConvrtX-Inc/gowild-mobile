...
class TrailSavedRouteCell: UITableViewCell {
    ...
    @IBOutlet weak var tryRouteBtn: LoadingButton!{
        didSet{
            tryRouteBtn.titleLabel?.font = Fonts.getSourceSansProSemiBoldOf(size: 10)
    ...
    @IBOutlet weak var saveBtn: LoadingButton!{
        didSet{
            saveBtn.titleLabel?.font = Fonts.getSourceSansProSemiBoldOf(size: 10)
            saveBtn.setTitleColor(AppColor.appWhiteColor(), for: .normal)
            saveBtn.backgroundColor = AppColor.textLightOrangeColor()
            saveBtn.setTitle(GoWildStrings.unSave(), for: .normal)
        }
    }
    @IBOutlet weak var detailBtn: LoadingButton!{
        didSet{
            detailBtn.titleLabel?.font = Fonts.getSourceSansProSemiBoldOf(size: 10)
    ...
    var arrayOfLeaderboard : [SaveRoutesLeaderboardData] = []{
        didSet{
...
extension TrailSavedRouteCell: ListViewMethods{
